/*
 * mixed_quad.c — Mixed-input 2x2 quad viewer with GPU undistortion
 *
 * Combines V4L2 raw (NVP6324 UYVY / UVC YUYV), V4L2 compressed decode
 * (MJPG/H264 via FFmpeg), and RTSP network streams (FFmpeg+RKMPP) into
 * a 2x2 DRM display with per-channel GPU barrel/pincushion correction.
 *
 * Two modes:
 *   MODE_QUAD   — 2x2 grid preview with blinking selection border
 *   MODE_SINGLE — fullscreen calibration for selected source
 *
 * Per-source calibration loaded/saved to /root/lens_calib_src{0..3}.conf
 *
 * Build (on board):
 *   gcc -O2 -o mixed_quad mixed_quad.c \
 *       $(pkg-config --cflags --libs librga libdrm gbm egl glesv2 libavformat libavcodec libavutil libswscale) \
 *       -lm -lpthread
 *
 * Usage:
 ...
 *
 * Keyboard (MODE_QUAD):
 *   Left/Right  = cycle selected source
 *   Enter       = enter fullscreen calibration for selected source
 *   Q           = quit
 *
...
enum src_type { SRC_NONE, SRC_V4L2, SRC_V4L2_DECODE, SRC_RTSP };

static struct {
    enum src_type type;
    const char   *path;    /* /dev/videoN or rtsp://... */
    int           width, height;
    const char   *label;
} inputs[MAX_SRCS] = {
...
static volatile int g_selected = 0;       /* 0-3: selected src in quad mode */
...
static volatile int g_screenshot_req = 0;
static void on_screenshot(int s) { (void)s; g_screenshot_req = 1; }
...
static void calib_path(char *buf, size_t sz, int src)
{
    snprintf(buf, sz, "/root/lens_calib_src%d.conf", src);
}
...
static void save_screenshot(void *map, uint32_t pitch, uint32_t w, uint32_t h)
{
    static int seq = 0;
    char path[64];
    snprintf(path, sizeof(path), "/tmp/mixed_quad_%03d.ppm", seq++);
    FILE *fp = fopen(path, "wb");
    if (!fp) { perror("screenshot fopen"); return; }
    fprintf(fp, "P6\n%u %u\n255\n", w, h);
    uint8_t *row = (uint8_t *)map;
...
struct source {
    ...
    /* Common */
    int width, height, src_height;
    uint32_t pixfmt;         /* V4L2_PIX_FMT_UYVY/YUYV/MJPEG/H264 or 0 */
    int rga_fmt;             /* RK_FORMAT_UYVY_422 / YUYV_422 / YCbCr_420_SP */
    im_rect quad;
    struct calib cal;

    /* V4L2 (SRC_V4L2 and SRC_V4L2_DECODE) */
    int fd;
    char dev_path[64];
    uint32_t buf_type;
    int is_mplane;
    ...
    /* RTSP URL (SRC_RTSP only) */
    char url[256];

    /* Decode thread (SRC_V4L2_DECODE and SRC_RTSP) */
    pthread_t dec_thread;
    int thread_started;

    /* NV12 double-buffered DRM dumb buffers (SRC_V4L2_DECODE and SRC_RTSP) */
    uint32_t nv12_gem[2];
    int nv12_dmabuf[2];
    void *nv12_map[2];
    uint32_t nv12_pitch;
    uint64_t nv12_size;
    rga_buffer_handle_t nv12_rga_handle[2];
    rga_buffer_t nv12_rga[2];
    int nv12_back;           /* writer index (thread) */
    volatile int new_frame;  /* flag: thread sets 1, main clears */
    pthread_mutex_t lock;
    int front_idx;           /* reader index (main loop) */

    /* GPU intermediate buffer */
    uint32_t inter_gem, inter_pitch;
    uint64_t inter_size;
    int inter_dmabuf_fd;
...
/* ─────────────────── Media pipeline auto-setup ─────────────────── */

static int setup_media_pipeline(void)
{
    /* Only run if NVP6324 sysfs exists */
    if (access(NVP_CHANNEL_FMT, F_OK) != 0) {
        printf("NVP6324 sysfs not found, skipping media pipeline setup\n");
        return 0;
    }

    /* Find the largest V4L2 resolution among configured sources */
    int max_w = 0, max_h = 0;
    for (int i = 0; i < MAX_SRCS; i++) {
        if (inputs[i].type != SRC_V4L2) continue;
        if (inputs[i].width > max_w)  max_w = inputs[i].width;
        if (inputs[i].height > max_h) max_h = inputs[i].height;
    }
    if (max_w == 0 || max_h == 0) {
        printf("No SRC_V4L2 sources, skipping media pipeline\n");
        return 0;
    }
...
/* V4L2 raw capture init (SRC_V4L2) — auto-detect UYVY vs YUYV */
static int src_v4l2_init(struct source *s, int idx)
{
    for (int i = 0; i < V4L2_BUF_CNT; i++) {
        s->bufs[i].dmabuf_fd = -1;
        s->bufs[i].start = NULL;
        s->bufs[i].rga_handle = 0;
    }

    s->fd = open(s->dev_path, O_RDWR | O_NONBLOCK);
    if (s->fd < 0) {
        fprintf(stderr, "V4L2: open %s: %s\n", s->dev_path, strerror(errno));
        return -1;
    }

    struct v4l2_capability cap;
    if (xioctl(s->fd, VIDIOC_QUERYCAP, &cap) < 0) goto err;

    s->is_mplane = !!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE_MPLANE);
    if (!s->is_mplane && !(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE))
        goto err;
    s->buf_type = s->is_mplane
        ? V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE
        : V4L2_BUF_TYPE_VIDEO_CAPTURE;

    int req_w = inputs[idx].width;
    int req_h = inputs[idx].height;

    /* Try UYVY first, then YUYV */
    uint32_t try_fmts[] = { V4L2_PIX_FMT_UYVY, V4L2_PIX_FMT_YUYV };
    int fmt_ok = 0;
    for (int f = 0; f < 2 && !fmt_ok; f++) {
        struct v4l2_format fmt = { .type = s->buf_type };
        if (s->is_mplane) {
            fmt.fmt.pix_mp.width       = req_w;
            fmt.fmt.pix_mp.height      = req_h;
            fmt.fmt.pix_mp.pixelformat = try_fmts[f];
            fmt.fmt.pix_mp.num_planes  = 1;
            fmt.fmt.pix_mp.field       = V4L2_FIELD_NONE;
        } else {
            fmt.fmt.pix.width       = req_w;
            fmt.fmt.pix.height      = req_h;
            fmt.fmt.pix.pixelformat = try_fmts[f];
            fmt.fmt.pix.field       = V4L2_FIELD_NONE;
        }
        if (xioctl(s->fd, VIDIOC_S_FMT, &fmt) < 0)
            continue;
        /* Verify the driver actually set the format we asked for */
        uint32_t got_fmt = s->is_mplane ? fmt.fmt.pix_mp.pixelformat : fmt.fmt.pix.pixelformat;
        if (got_fmt != try_fmts[f])
            continue;
        s->pixfmt = try_fmts[f];
        s->width  = s->is_mplane ? fmt.fmt.pix_mp.width  : fmt.fmt.pix.width;
        s->height = s->is_mplane ? fmt.fmt.pix_mp.height : fmt.fmt.pix.height;
        fmt_ok = 1;
    }
    if (!fmt_ok) {
        fprintf(stderr, "V4L2: %s: no UYVY or YUYV format\n", s->dev_path);
        goto err;
    }

    if (s->pixfmt == V4L2_PIX_FMT_UYVY)
        s->rga_fmt = RK_FORMAT_UYVY_422;
    else
        s->rga_fmt = RK_FORMAT_YUYV_422;

    s->src_height = s->height;

    printf("V4L2 raw: %s -> %dx%d %s (src_h=%d)\n",
           s->dev_path, s->width, s->height,
           s->pixfmt == V4L2_PIX_FMT_UYVY ? "UYVY" : "YUYV",
           s->src_height);
    ...
    if (xioctl(s->fd, VIDIOC_REQBUFS, &reqb) < 0) {
        fprintf(stderr, "V4L2: %s REQBUFS: %s\n", s->dev_path, strerror(errno));
        goto err;
    }
    s->buf_count = reqb.count;
    ...
    for (int i = 0; i < s->buf_count; i++) {
        ...
        if (s->is_mplane) { vb.m.planes = planes; vb.length = 1; }

        if (xioctl(s->fd, VIDIOC_QUERYBUF, &vb) < 0) goto err;

        uint32_t off = s->is_mplane ? planes[0].m.mem_offset : vb.m.offset;
        size_t   len = s->is_mplane ? planes[0].length       : vb.length;

        s->bufs[i].start = mmap(NULL, len, PROT_READ | PROT_WRITE,
                                MAP_SHARED, s->fd, off);
        if (s->bufs[i].start == MAP_FAILED) {
            s->bufs[i].start = NULL;
            goto err;
        }
        s->bufs[i].length = len;
        ...
        if (xioctl(s->fd, VIDIOC_EXPBUF, &eb) < 0) {
            fprintf(stderr, "V4L2: %s EXPBUF %d: %s\n", s->dev_path, i, strerror(errno));
            goto err;
        }
        s->bufs[i].dmabuf_fd = eb.fd;
    }
    ...
    for (int i = 0; i < s->buf_count; i++) {
        ...
        struct v4l2_plane planes[1] = {};
        if (s->is_mplane) { vb.m.planes = planes; vb.length = 1; }
        if (xioctl(s->fd, VIDIOC_QBUF, &vb) < 0) goto err;
    }

    int type_on = s->buf_type;
    if (xioctl(s->fd, VIDIOC_STREAMON, &type_on) < 0) {
...
err:
    for (int i = 0; i < V4L2_BUF_CNT; i++) {
        if (s->bufs[i].dmabuf_fd >= 0) { close(s->bufs[i].dmabuf_fd); s->bufs[i].dmabuf_fd = -1; }
        if (s->bufs[i].start)          { munmap(s->bufs[i].start, s->bufs[i].length); s->bufs[i].start = NULL; }
    }
    if (s->fd >= 0) { close(s->fd); s->fd = -1; }
    return -1;
}

/* V4L2 decode init (SRC_V4L2_DECODE) — try H264 then MJPEG */
static int src_v4l2_decode_init(struct source *s, int idx)
{
    for (int i = 0; i < V4L2_BUF_CNT; i++) {
        s->bufs[i].dmabuf_fd = -1;
        s->bufs[i].start = NULL;
    ...
    s->fd = open(s->dev_path, O_RDWR | O_NONBLOCK);
    if (s->fd < 0) {
        fprintf(stderr, "V4L2dec: open %s: %s\n", s->dev_path, strerror(errno));
        return -1;
    }
    ...
    /* Try H264 first, then MJPEG, then YUYV/UYVY as fallback */
    uint32_t try_fmts[] = { V4L2_PIX_FMT_H264, V4L2_PIX_FMT_MJPEG,
                            V4L2_PIX_FMT_YUYV, V4L2_PIX_FMT_UYVY };
    int fmt_ok = 0;
    for (int f = 0; f < 4 && !fmt_ok; f++) {
        struct v4l2_format fmt = { .type = s->buf_type };
        if (s->is_mplane) {
            fmt.fmt.pix_mp.width       = req_w;
            fmt.fmt.pix_mp.height      = req_h;
    ...
    if (!fmt_ok) {
        fprintf(stderr, "V4L2dec: %s: no H264/MJPEG/YUYV/UYVY\n", s->dev_path);
        goto err;
    }

    /* If we got a raw format, treat as SRC_V4L2 instead */
    if (s->pixfmt == V4L2_PIX_FMT_YUYV || s->pixfmt == V4L2_PIX_FMT_UYVY) {
        printf("V4L2dec: %s: got raw format, switching to SRC_V4L2 path\n", s->dev_path);
        close(s->fd);
        s->fd = -1;
        s->type = SRC_V4L2;
        s->rga_fmt = (s->pixfmt == V4L2_PIX_FMT_UYVY)
                     ? RK_FORMAT_UYVY_422 : RK_FORMAT_YUYV_422;
        return src_v4l2_init(s, idx);
    }

    s->rga_fmt = RK_FORMAT_YCbCr_420_SP;
    s->src_height = s->height;

    printf("V4L2dec: %s -> %dx%d %s\n",
           s->dev_path, s->width, s->height,
           s->pixfmt == V4L2_PIX_FMT_H264 ? "H264" : "MJPEG");

    /* Enable dynamic framerate (allows longer exposure in low light) */
    struct v4l2_control dynfr = { .id = V4L2_CID_EXPOSURE_AUTO_PRIORITY, .value = 1 };
    if (xioctl(s->fd, VIDIOC_S_CTRL, &dynfr) == 0)
        printf("V4L2dec: %s: exposure_dynamic_framerate enabled\n", s->dev_path);
    ...
    if (xioctl(s->fd, VIDIOC_REQBUFS, &reqb) < 0) {
        fprintf(stderr, "V4L2dec: %s REQBUFS: %s\n", s->dev_path, strerror(errno));
        goto err;
    }
    s->buf_count = reqb.count;
    ...
    int type_on = s->buf_type;
    if (xioctl(s->fd, VIDIOC_STREAMON, &type_on) < 0) {
        fprintf(stderr, "V4L2dec: %s STREAMON: %s\n", s->dev_path, strerror(errno));
        goto err;
    }
...
static void src_v4l2_cleanup(struct source *s)
{
    if (s->fd < 0) return;
    int type = s->buf_type;
    xioctl(s->fd, VIDIOC_STREAMOFF, &type);
    for (int i = 0; i < s->buf_count; i++) {
        if (s->bufs[i].rga_handle) { releasebuffer_handle(s->bufs[i].rga_handle); s->bufs[i].rga_handle = 0; }
        if (s->bufs[i].dmabuf_fd >= 0) { close(s->bufs[i].dmabuf_fd); s->bufs[i].dmabuf_fd = -1; }
        if (s->bufs[i].start) { munmap(s->bufs[i].start, s->bufs[i].length); s->bufs[i].start = NULL; }
    }
    close(s->fd);
...
/* Try to reconnect a V4L2 source — returns 0 on success */
static int src_v4l2_reconnect(struct source *s, int idx)
{
    src_v4l2_cleanup(s);
    s->active = 0;

    if (src_v4l2_init(s, idx) != 0)
        return -1;

    /* Re-import RGA handles for the new dmabuf fds */
    im_handle_param_t sp = {
        .width  = (uint32_t)s->width,
        .height = (uint32_t)s->height,
        .format = s->rga_fmt,
    };
    for (int b = 0; b < s->buf_count; b++) {
        s->bufs[b].rga_handle = importbuffer_fd(s->bufs[b].dmabuf_fd, &sp);
        if (!s->bufs[b].rga_handle) {
            fprintf(stderr, "V4L2 reconnect: RGA import src%d buf%d failed\n", idx, b);
            src_v4l2_cleanup(s);
            return -1;
        }
        s->bufs[b].rga = wrapbuffer_handle(s->bufs[b].rga_handle,
                                            s->width, s->height,
                                            s->rga_fmt);
        s->bufs[b].rga.color_space_mode = IM_YUV_TO_RGB_BT601_LIMIT;
    }

    printf("V4L2 reconnect: src%d %s OK\n", idx, s->dev_path);
    return 0;
}

/* ─────────────────── NV12 dumb buffer allocation ─────────────────── */

static int alloc_nv12_dumb(struct source *s, int drm_fd)
{
    int w = s->width;
    int h = s->height;
    int nv12_h = h * 3 / 2;  /* Y (h lines) + UV (h/2 lines) */

    for (int b = 0; b < 2; b++) {
        s->nv12_dmabuf[b] = -1;
        s->nv12_map[b] = NULL;
        s->nv12_gem[b] = 0;
        s->nv12_rga_handle[b] = 0;

        struct drm_mode_create_dumb cr = {
            .width  = (uint32_t)w,
            .height = (uint32_t)nv12_h,
            .bpp    = 8,
        };
        if (drmIoctl(drm_fd, DRM_IOCTL_MODE_CREATE_DUMB, &cr)) {
            perror("NV12 create_dumb");
            return -1;
        }
        s->nv12_gem[b] = cr.handle;
        s->nv12_pitch  = cr.pitch;
        s->nv12_size   = cr.size;

        struct drm_mode_map_dumb mr = { .handle = cr.handle };
        if (drmIoctl(drm_fd, DRM_IOCTL_MODE_MAP_DUMB, &mr)) {
            perror("NV12 map_dumb");
            return -1;
        }
        s->nv12_map[b] = mmap(NULL, cr.size, PROT_READ | PROT_WRITE,
                               MAP_SHARED, drm_fd, mr.offset);
        if (s->nv12_map[b] == MAP_FAILED) {
            s->nv12_map[b] = NULL;
            return -1;
        }

        /* Initialize to black NV12: Y=0, UV=128 */
        memset(s->nv12_map[b], 0, s->nv12_pitch * h);
        memset((uint8_t *)s->nv12_map[b] + s->nv12_pitch * h,
               128, s->nv12_pitch * h / 2);

        if (drmPrimeHandleToFD(drm_fd, cr.handle,
                               DRM_CLOEXEC | DRM_RDWR, &s->nv12_dmabuf[b])) {
            perror("NV12 primeHandleToFD");
            return -1;
        }

        /* Import to RGA as NV12 */
        im_handle_param_t ip = {
            .width  = (uint32_t)s->nv12_pitch,
            .height = (uint32_t)h,
            .format = RK_FORMAT_YCbCr_420_SP,
        };
        s->nv12_rga_handle[b] = importbuffer_fd(s->nv12_dmabuf[b], &ip);
        if (!s->nv12_rga_handle[b]) {
            fprintf(stderr, "NV12 RGA import buf%d failed\n", b);
            return -1;
        }
        s->nv12_rga[b] = wrapbuffer_handle(s->nv12_rga_handle[b],
                                             w, h,
                                             RK_FORMAT_YCbCr_420_SP,
                                             s->nv12_pitch, h);
        s->nv12_rga[b].color_space_mode = IM_YUV_TO_RGB_BT601_LIMIT;
    }
    ...
    printf("NV12 dumb: src%d %dx%d pitch=%u (%uKB x2)\n",
           (int)(s - (struct source *)NULL), /* placeholder, caller prints idx */
           w, h, s->nv12_pitch, (unsigned)(s->nv12_size / 1024));
    return 0;
}

static void free_nv12_dumb(struct source *s, int drm_fd)
{
    for (int b = 0; b < 2; b++) {
        if (s->nv12_rga_handle[b]) releasebuffer_handle(s->nv12_rga_handle[b]);
        if (s->nv12_dmabuf[b] >= 0) close(s->nv12_dmabuf[b]);
        if (s->nv12_map[b]) munmap(s->nv12_map[b], s->nv12_size);
        if (s->nv12_gem[b]) {
            struct drm_mode_destroy_dumb dd = { .handle = s->nv12_gem[b] };
...
/* ─────────────────── Copy NV12 frame to dumb buffer ─────────────────── */

/* Copy from AVFrame (planar NV12 with stride) to dumb buffer (contiguous NV12 with pitch) */
static void copy_nv12_to_dumb(struct source *s, const AVFrame *frame)
{
    int w = s->width;
    int h = s->height;
    int back = s->nv12_back;
    uint8_t *dst = (uint8_t *)s->nv12_map[back];

    /* Y plane: frame->data[0], linesize[0] -> dst, pitch */
    const uint8_t *ysrc = frame->data[0];
    int ystride = frame->linesize[0];
    for (int y = 0; y < h; y++)
        memcpy(dst + y * s->nv12_pitch, ysrc + y * ystride,
               w < ystride ? w : ystride);

    /* UV plane: frame->data[1], linesize[1] -> dst + pitch*h, pitch */
    const uint8_t *uvsrc = frame->data[1];
    int uvstride = frame->linesize[1];
    uint8_t *uvdst = dst + s->nv12_pitch * h;
    for (int y = 0; y < h / 2; y++)
        memcpy(uvdst + y * s->nv12_pitch, uvsrc + y * uvstride,
               w < uvstride ? w : uvstride);
}

/* Copy from YUV420P (planar with separate U and V) to NV12 dumb buffer */
static void copy_yuv420p_to_nv12_dumb(struct source *s, const AVFrame *frame)
{
    int w = s->width;
    int h = s->height;
    int back = s->nv12_back;
    ...
    int ystride = frame->linesize[0];
    for (int y = 0; y < h; y++)
        memcpy(dst + y * s->nv12_pitch, ysrc + y * ystride, w);

    /* Interleave U and V to NV12 UV plane */
    ...
    int cw = w / 2;
    for (int y = 0; y < h / 2; y++) {
        uint8_t *row = uvdst + y * s->nv12_pitch;
        const uint8_t *urow = usrc + y * ustride;
        const uint8_t *vrow = vsrc + y * vstride;
        for (int x = 0; x < cw; x++) {
            row[x * 2 + 0] = urow[x];
            row[x * 2 + 1] = vrow[x];
        }
...
/* ─────────────────── Decode thread (RTSP + V4L2_DECODE) ─────────────────── */

static void *decode_thread(void *arg)
{
    struct source *s = (struct source *)arg;
    int is_rtsp = (s->type == SRC_RTSP);
    char src_name[64];
    snprintf(src_name, sizeof(src_name), "[%s %s]",
             is_rtsp ? "RTSP" : "V4L2dec", s->label);
    ...
    while (!quit) {
        ...
        if (is_rtsp) {
            /* ── RTSP: open network stream ── */
            AVDictionary *opts = NULL;
            av_dict_set(&opts, "rtsp_transport", "tcp", 0);
            av_dict_set(&opts, "stimeout", "5000000", 0);

            int ret = avformat_open_input(&fmt_ctx, s->url, NULL, &opts);
            av_dict_free(&opts);
            if (ret < 0) {
                char errbuf[128];
            ...
            /* Try RKMPP hardware decoder */
            const AVCodec *codec = NULL;
            const char *hw_names[] = { "h264_rkmpp", "hevc_rkmpp", NULL };
            for (const char **n = hw_names; *n && !codec; n++) {
                if (par->codec_id == AV_CODEC_ID_H264 && strstr(*n, "h264"))
                    codec = avcodec_find_decoder_by_name(*n);
                else if (par->codec_id == AV_CODEC_ID_HEVC && strstr(*n, "hevc"))
                    codec = avcodec_find_decoder_by_name(*n);
            }
            if (codec) {
                hw_decode = 1;
                printf("%s using HW decoder: %s\n", src_name, codec->name);
            } else {
                codec = avcodec_find_decoder(par->codec_id);
                if (!codec) {
                    fprintf(stderr, "%s no decoder for codec %d\n", src_name, par->codec_id);
                    goto reconnect;
                }
                hw_decode = 0;
                printf("%s using SW decoder: %s\n", src_name, codec->name);
            }
            ...
            if (hw_decode) {
                int hret = av_hwdevice_ctx_create(&hw_device_ctx,
                                                   AV_HWDEVICE_TYPE_RKMPP,
                                                   NULL, NULL, 0);
                if (hret < 0) {
                    fprintf(stderr, "%s RKMPP device failed, falling back to SW\n", src_name);
                    avcodec_free_context(&dec_ctx);
                    codec = avcodec_find_decoder(par->codec_id);
                    if (!codec) goto reconnect;
                    dec_ctx = avcodec_alloc_context3(codec);
                    avcodec_parameters_to_context(dec_ctx, par);
                    hw_decode = 0;
                } else {
            ...
            if (dec_ctx->width > 0 && dec_ctx->height > 0) {
                ...
                s->src_height = dec_ctx->height;
            }
            printf("%s stream: %dx%d\n", src_name, s->width, s->height);

        } else {
            /* ── V4L2_DECODE: build FFmpeg decoder from V4L2 pixel format ── */
            const AVCodec *codec = NULL;
            enum AVCodecID codec_id;

            if (s->pixfmt == V4L2_PIX_FMT_H264)
                codec_id = AV_CODEC_ID_H264;
            else
                codec_id = AV_CODEC_ID_MJPEG;

            /* Try HW decoder for H264 */
            ...
            if (!codec) {
                fprintf(stderr, "%s no decoder\n", src_name);
                goto reconnect;
            }

            dec_ctx = avcodec_alloc_context3(codec);
            dec_ctx->width = s->width;
            dec_ctx->height = s->height;
        ...
        while (!quit) {
            ...
            if (is_rtsp) {
                ...
                /* V4L2_DECODE: read a compressed frame from V4L2 */
                struct v4l2_buffer vb = {
                    .type   = s->buf_type,
                    .memory = V4L2_MEMORY_MMAP,
                };
                ...
                if (xioctl(s->fd, VIDIOC_DQBUF, &vb) < 0) continue;

                size_t bytesused = s->is_mplane ? planes[0].bytesused : vb.bytesused;
                if (bytesused > 0) {
                    pkt->data = (uint8_t *)s->bufs[vb.index].start;
                    pkt->size = (int)bytesused;
                    got_pkt = 1;
                }

                /* Must requeue before continue */
                int requeue_idx = vb.index;

                if (!got_pkt) {
                    xioctl(s->fd, VIDIOC_QBUF, &vb);
                    continue;
                }

                /* Send packet to decoder */
                int sret = avcodec_send_packet(dec_ctx, pkt);
                /* Clear packet refs since data is from mmap (not allocated) */
                pkt->data = NULL;
                pkt->size = 0;

                /* Requeue V4L2 buffer now */
                xioctl(s->fd, VIDIOC_QBUF, &vb);
                (void)requeue_idx;
                ...
                while (!quit) {
                    int rret = avcodec_receive_frame(dec_ctx, frame);
                    if (rret < 0) break;

                    AVFrame *out = frame;
                    if (hw_decode && frame->format == AV_PIX_FMT_DRM_PRIME) {
                    ...
                    /* Copy to NV12 dumb buffer */
                    if (out->format == AV_PIX_FMT_NV12) {
                        copy_nv12_to_dumb(s, out);
                    } else if (out->format == AV_PIX_FMT_YUV420P ||
                               out->format == AV_PIX_FMT_YUVJ420P) {
                        copy_yuv420p_to_nv12_dumb(s, out);
                    }

                    /* Swap front/back, signal new frame */
                    pthread_mutex_lock(&s->lock);
                    s->front_idx = s->nv12_back;
                    s->nv12_back = 1 - s->nv12_back;
                    s->new_frame = 1;
                    pthread_mutex_unlock(&s->lock);

                    av_frame_unref(frame);
                }
                continue;  /* Skip the RTSP send/receive below */
            }

            /* RTSP path: send/receive */
            if (got_pkt) {
                int sret = avcodec_send_packet(dec_ctx, pkt);
                av_packet_unref(pkt);
                if (sret < 0) continue;
...
reconnect:
        ...
        if (is_rtsp) {
            fprintf(stderr, "%s reconnecting in 3s...\n", src_name);
            for (int w = 0; w < 30 && !quit; w++)
                usleep(100000);
        } else {
            /* V4L2_DECODE: don't reconnect, just exit thread */
            fprintf(stderr, "%s decode error, exiting thread\n", src_name);
            break;
        }
    }
...
static int rga_setup(struct source srcs[], int nsrcs, struct display *d)
{
    ...
    for (int i = 0; i < nsrcs; i++) {
        ...
        s->quad = (im_rect){ pos[i][0], pos[i][1], qw, qh };
        printf("RGA: src%d %dx%d -> quad (%d,%d)+%dx%d\n",
               i, s->width, s->height,
               s->quad.x, s->quad.y, qw, qh);

        if (s->type == SRC_V4L2) {
            /* Import V4L2 mmap dmabuf fds with per-source format */
            im_handle_param_t sp = {
                .width  = (uint32_t)s->width,
                .height = (uint32_t)s->height,
                .format = s->rga_fmt,
            ...
            for (int b = 0; b < s->buf_count; b++) {
                s->bufs[b].rga_handle = importbuffer_fd(s->bufs[b].dmabuf_fd, &sp);
                if (!s->bufs[b].rga_handle) {
                    fprintf(stderr, "RGA: import src%d buf%d failed\n", i, b);
                    return -1;
                }
                s->bufs[b].rga = wrapbuffer_handle(s->bufs[b].rga_handle,
                                                    s->width, s->height,
        ...
        /* SRC_V4L2_DECODE / SRC_RTSP: NV12 dumb buffers already imported in alloc_nv12_dumb */
    }
    return 0;
}
...
/* Create per-source intermediate buffer + GL source texture */
static int gpu_init_src_texture(gpu_ctx *g, struct source *s, int drm_fd)
{
    s->inter_dmabuf_fd = -1;
    s->inter_gem = 0;
    s->src_img = EGL_NO_IMAGE_KHR;
    s->src_tex = 0;

    struct drm_mode_create_dumb cr = {
        .width = (uint32_t)s->width, .height = (uint32_t)s->height, .bpp = 32,
    };
    if (drmIoctl(drm_fd, DRM_IOCTL_MODE_CREATE_DUMB, &cr)) {
        perror("GPU: inter create_dumb"); return -1;
    }
    s->inter_gem = cr.handle;
    s->inter_pitch = cr.pitch;
    s->inter_size = cr.size;
    ...
    int iwstride = (int)(cr.pitch / 4);
    im_handle_param_t ip = {
        .width = (uint32_t)iwstride, .height = (uint32_t)s->height,
        .format = RK_FORMAT_BGRX_8888,
    };
    s->inter_rga_handle = importbuffer_fd(s->inter_dmabuf_fd, &ip);
    if (!s->inter_rga_handle) {
        fprintf(stderr, "GPU: src inter RGA import failed\n"); return -1;
    }
    s->inter_rga = wrapbuffer_handle(s->inter_rga_handle,
                                      s->width, s->height,
                                      RK_FORMAT_BGRX_8888, iwstride, s->height);

    EGLint attrs[] = {
        EGL_WIDTH, s->width,
        EGL_HEIGHT, s->height,
        EGL_LINUX_DRM_FOURCC_EXT, DRM_FORMAT_XRGB8888,
        EGL_DMA_BUF_PLANE0_FD_EXT, s->inter_dmabuf_fd,
        EGL_DMA_BUF_PLANE0_OFFSET_EXT, 0,
        EGL_DMA_BUF_PLANE0_PITCH_EXT, (EGLint)cr.pitch,
        EGL_NONE,
    };
    s->src_img = g->CreateImage(g->dpy, EGL_NO_CONTEXT,
                                 EGL_LINUX_DMA_BUF_EXT, NULL, attrs);
    if (s->src_img == EGL_NO_IMAGE_KHR) {
        fprintf(stderr, "GPU: src EGLImage failed\n"); return -1;
    }

    glGenTextures(1, &s->src_tex);
    glBindTexture(GL_TEXTURE_2D, s->src_tex);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    g->ImageTargetTex2D(GL_TEXTURE_2D, (GLeglImageOES)s->src_img);

    printf("GPU: src inter buf %dx%d pitch=%u (%uKB)\n",
           s->width, s->height, cr.pitch, (unsigned)(cr.size / 1024));
    return 0;
}

static void gpu_cleanup_src_texture(gpu_ctx *g, struct source *s, int drm_fd)
{
    if (s->src_tex) { glDeleteTextures(1, &s->src_tex); s->src_tex = 0; }
    if (s->src_img != EGL_NO_IMAGE_KHR) {
        g->DestroyImage(g->dpy, s->src_img);
        s->src_img = EGL_NO_IMAGE_KHR;
    }
    if (s->inter_rga_handle) { releasebuffer_handle(s->inter_rga_handle); s->inter_rga_handle = 0; }
    if (s->inter_dmabuf_fd >= 0) { close(s->inter_dmabuf_fd); s->inter_dmabuf_fd = -1; }
    if (s->inter_gem) {
        struct drm_mode_destroy_dumb dd = { .handle = s->inter_gem };
        drmIoctl(drm_fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dd);
        s->inter_gem = 0;
    }
}
...
static void draw_osd_single(void *map, int pitch, int disp_w, int disp_h,
                             double fps, int src_idx, const char *label,
                             double k1, double k2,
                             double cx, double cy, int grid_on, int bypass)
{
    ...
    char line[80];
    snprintf(line, sizeof(line), "=== src%d %s [%.0f fps] ===", src_idx, label, fps);
...
/* Quad labels showing source label and type */
static void draw_quad_labels(uint32_t *fb, int pitch4, int disp_w, int disp_h,
                             struct source srcs[], int nsrcs)
{
    int qw = disp_w / 2, qh = disp_h / 2;
    int pos[4][2] = { {0,0}, {qw,0}, {0,qh}, {qw,qh} };
    const char *type_str[] = { "?", "V4L2", "DEC", "RTSP" };
    for (int i = 0; i < nsrcs; i++) {
        if (!srcs[i].active) continue;
        char label[48];
        snprintf(label, sizeof(label), "%s [%s]",
                 srcs[i].label, type_str[srcs[i].type]);
        ...
        osd_str(fb, pitch4, lx, ly, label,
                srcs[i].cal.k1 != 0.0 || srcs[i].cal.k2 != 0.0
...
static int active_srcs[MAX_SRCS];
...
static void handle_action(int action)
{
    ...
    if (mode == MODE_QUAD) {
        ...
        case 'r': {
            ...
            for (int i = 0; i < num_active; i++) {
                if (active_srcs[i] == cur) {
                    g_selected = active_srcs[(i + 1) % num_active];
                    break;
                }
            }
            break;
        ...
        case 'l': {
            ...
            for (int i = 0; i < num_active; i++) {
                if (active_srcs[i] == cur) {
                    g_selected = active_srcs[(i - 1 + num_active) % num_active];
                    break;
                }
            }
            break;
    ...
    if (action == 'P') g_screenshot_req = 1;
                               ...
                               float w, float h, float max_r)
{
    ...
    if (min_s < 0.1f) min_s = 0.1f;
    return min_s;
}

/* ─────────────────── Auto edge detection for calibration ─────────────────── */

/* Takes a Y-plane buffer (extracted from any format) */
static void auto_detect_edges(const uint8_t *Y, int w, int h)
{
    printf("Auto-detect: analyzing %dx%d frame...\n", w, h);

    /* Light 3x3 box blur to reduce noise */
    ...
    /* Compute Sobel Gy (horizontal edges) and Gx (vertical edges) */
...
/* Extract Y plane for auto-detect based on source type */
static uint8_t *extract_y_for_autodetect(struct source *s, int buf_idx)
{
    int w = s->width;
    int h = s->src_height;
    ...
    if (s->type == SRC_V4L2) {
        const uint8_t *raw = (const uint8_t *)s->bufs[buf_idx].start;
        if (s->pixfmt == V4L2_PIX_FMT_UYVY) {
            /* UYVY: Y at byte offsets 1,3,5,7... */
            int bstride = w * 2;
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                    Y[y * w + x] = raw[y * bstride + x * 2 + 1];
        } else {
            /* YUYV: Y at byte offsets 0,2,4,6... */
            int bstride = w * 2;
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                    Y[y * w + x] = raw[y * bstride + x * 2];
        }
    } else {
        /* NV12: Y plane is at the start of the dumb buffer */
        int front;
        pthread_mutex_lock(&s->lock);
        front = s->front_idx;
        pthread_mutex_unlock(&s->lock);
        const uint8_t *nv12 = (const uint8_t *)s->nv12_map[front];
        for (int y = 0; y < h; y++)
            memcpy(Y + y * w, nv12 + y * s->nv12_pitch, w);
    }
    return Y;
}
...
int main(int argc, char **argv)
{
    ...
    signal(SIGUSR1, on_screenshot);

    /* ── Media pipeline (NVP6324 only if sysfs exists) ── */
    ...
    /* ── Sources ── */
    struct source srcs[MAX_SRCS];
    int n_active = 0;
    for (int i = 0; i < MAX_SRCS; i++) {
        memset(&srcs[i], 0, sizeof(srcs[i]));
        srcs[i].fd = -1;
        srcs[i].inter_dmabuf_fd = -1;
        srcs[i].nv12_dmabuf[0] = srcs[i].nv12_dmabuf[1] = -1;
        srcs[i].src_img = EGL_NO_IMAGE_KHR;
        srcs[i].type = inputs[i].type;
        snprintf(srcs[i].label, sizeof(srcs[i].label), "%s", inputs[i].label);

        if (inputs[i].type == SRC_NONE) continue;

        srcs[i].width = inputs[i].width;
        srcs[i].height = inputs[i].height;
        srcs[i].src_height = inputs[i].height;

        if (inputs[i].type == SRC_V4L2) {
            snprintf(srcs[i].dev_path, sizeof(srcs[i].dev_path), "%s", inputs[i].path);
            if (src_v4l2_init(&srcs[i], i) == 0) {
                n_active++;
                printf("src%d: V4L2 raw %s active\n", i, srcs[i].dev_path);
            } else {
                fprintf(stderr, "src%d: V4L2 %s failed\n", i, inputs[i].path);
            }
        } else if (inputs[i].type == SRC_V4L2_DECODE) {
            snprintf(srcs[i].dev_path, sizeof(srcs[i].dev_path), "%s", inputs[i].path);
            if (src_v4l2_decode_init(&srcs[i], i) == 0) {
                /* If type got switched to SRC_V4L2 in init, it's already active */
                if (srcs[i].type == SRC_V4L2) {
                    n_active++;
                    printf("src%d: V4L2 raw (auto-switched) %s active\n", i, srcs[i].dev_path);
                } else {
                    /* Allocate NV12 dumb buffers for decode output */
                    if (alloc_nv12_dumb(&srcs[i], disp.fd) < 0) {
                        fprintf(stderr, "src%d: NV12 alloc failed\n", i);
                        src_v4l2_cleanup(&srcs[i]);
                        srcs[i].active = 0;
                    } else {
                        n_active++;
                        printf("src%d: V4L2 decode %s active\n", i, srcs[i].dev_path);
                    }
                }
            } else {
                fprintf(stderr, "src%d: V4L2dec %s failed\n", i, inputs[i].path);
            }
        } else if (inputs[i].type == SRC_RTSP) {
            snprintf(srcs[i].url, sizeof(srcs[i].url), "%s", inputs[i].path);
            srcs[i].rga_fmt = RK_FORMAT_YCbCr_420_SP;
            if (alloc_nv12_dumb(&srcs[i], disp.fd) < 0) {
                fprintf(stderr, "src%d: NV12 alloc failed\n", i);
            } else {
                srcs[i].active = 1;
                n_active++;
                printf("src%d: RTSP %s active\n", i, srcs[i].url);
            }
        }
    }
    if (!n_active) {
        fprintf(stderr, "No sources active\n");
        drm_cleanup(&disp);
        return 1;
    }
    printf("%d source(s) active\n", n_active);

    /* Build active source list for input thread cycling */
    num_active = 0;
    for (int i = 0; i < MAX_SRCS; i++)
        if (srcs[i].active) active_srcs[num_active++] = i;
    g_selected = active_srcs[0];

    /* ── RGA ── */
    if (rga_setup(srcs, MAX_SRCS, &disp) < 0) {
        for (int i = 0; i < MAX_SRCS; i++) {
            src_v4l2_cleanup(&srcs[i]);
            if (srcs[i].type == SRC_V4L2_DECODE || srcs[i].type == SRC_RTSP)
                free_nv12_dumb(&srcs[i], disp.fd);
        }
        drm_cleanup(&disp);
        return 1;
    }
    ...
    /* Load per-source calibration + create GPU textures if needed */
    for (int i = 0; i < MAX_SRCS; i++) {
        if (!srcs[i].active) continue;
        srcs[i].cal = (struct calib){ .k1 = 0, .k2 = 0, .cx = 0.5, .cy = 0.5, .zoom = MIN_ZOOM };
        char path[64];
        calib_path(path, sizeof(path), i);
        load_params(path, &srcs[i].cal);

        if (srcs[i].cal.k1 != 0.0 || srcs[i].cal.k2 != 0.0) {
            if (gpu_init_src_texture(&gpu, &srcs[i], disp.fd) < 0) {
                fprintf(stderr, "GPU: src%d texture init failed, using bypass\n", i);
                srcs[i].cal.k1 = 0;
                srcs[i].cal.k2 = 0;
            } else {
                printf("GPU: src%d undistort active (k1=%.4f k2=%.4f)\n",
                       i, srcs[i].cal.k1, srcs[i].cal.k2);
            }
        }
    }

    /* Start decode threads for V4L2_DECODE and RTSP sources */
    for (int i = 0; i < MAX_SRCS; i++) {
        if (!srcs[i].active) continue;
        if (srcs[i].type == SRC_V4L2_DECODE || srcs[i].type == SRC_RTSP) {
            if (pthread_create(&srcs[i].dec_thread, NULL, decode_thread, &srcs[i]) == 0) {
                srcs[i].thread_started = 1;
                printf("src%d: decode thread started\n", i);
            } else {
                fprintf(stderr, "src%d: failed to create decode thread\n", i);
            }
        }
    }
    ...
    unsigned long per_src_cnt[MAX_SRCS] = {0};
    int last_bi[MAX_SRCS] = {0};
    double fps = 0;
    ...
    while (!quit) {
        ...
        if (mode == MODE_QUAD && g_enter_single) {
            ...
            if (srcs[sel].active) {
                if (!srcs[sel].src_tex) {
                    if (gpu_init_src_texture(&gpu, &srcs[sel], disp.fd) < 0) {
                        fprintf(stderr, "GPU: src%d texture init failed\n", sel);
                        continue;
                    }
                    printf("GPU: src%d texture lazy-init OK\n", sel);
                }
                pthread_mutex_lock(&param_lock);
                g_k1 = srcs[sel].cal.k1;
                g_k2 = srcs[sel].cal.k2;
                g_cx = srcs[sel].cal.cx;
                g_cy = srcs[sel].cal.cy;
                g_grid = 0;
                g_bypass = 0;
                g_zoom = srcs[sel].cal.zoom >= MIN_ZOOM ? srcs[sel].cal.zoom : MIN_ZOOM;
                g_cursor_visible = 0;
                g_src_w = srcs[sel].width;
                g_src_h = srcs[sel].src_height;
                g_crop_x = 0; g_crop_y = 0;
                g_crop_w = srcs[sel].width;
                g_crop_h = srcs[sel].src_height;
                calib_npts = 0;
                calib_line = 0;
                g_mode = MODE_SINGLE;
                pthread_mutex_unlock(&param_lock);
                mode = MODE_SINGLE;
                printf("=== MODE_SINGLE: src%d %s fullscreen calibration ===\n",
                       sel, srcs[sel].label);
            }
        }
        ...
        if (mode == MODE_SINGLE && g_exit_single) {
            ...
            int sel = g_selected;
            pthread_mutex_lock(&param_lock);
            srcs[sel].cal.k1 = g_k1;
            srcs[sel].cal.k2 = g_k2;
            srcs[sel].cal.cx = g_cx;
            srcs[sel].cal.cy = g_cy;
            srcs[sel].cal.zoom = g_zoom;
            g_mode = MODE_QUAD;
            pthread_mutex_unlock(&param_lock);
            mode = MODE_QUAD;
        ...
        /* ── MODE_QUAD: poll V4L2 sources + check decode flags, draw 2x2 ── */
        if (mode == MODE_QUAD) {
            /* Build poll list from SRC_V4L2 sources only */
            struct pollfd pfds[MAX_SRCS];
            int pfd_to_src[MAX_SRCS];
            int nfds = 0;
            int have_decode_frame = 0;

            for (int i = 0; i < MAX_SRCS; i++) {
                if (!srcs[i].active) continue;
                if (srcs[i].type == SRC_V4L2) {
                    pfds[nfds].fd      = srcs[i].fd;
                    pfds[nfds].events  = POLLIN;
                    pfds[nfds].revents = 0;
                    pfd_to_src[nfds]   = i;
                    nfds++;
                } else if (srcs[i].new_frame) {
                    have_decode_frame = 1;
                }
            }

            /* Poll V4L2: timeout=0 if decode frame ready, else 16ms */
            int timeout_ms = have_decode_frame ? 0 : 16;
            int ret = 0;
            if (nfds > 0) {
                ret = poll(pfds, nfds, timeout_ms);
                if (ret < 0) { if (errno == EINTR) continue; break; }
            } else {
                /* No V4L2 sources, just wait a bit for decode frames */
                if (!have_decode_frame) {
                    usleep(5000);
                    /* Re-check decode flags */
                    have_decode_frame = 0;
                    for (int i = 0; i < MAX_SRCS; i++)
                        if (srcs[i].active && srcs[i].type != SRC_V4L2 && srcs[i].new_frame)
                            have_decode_frame = 1;
                    if (!have_decode_frame) goto fps_check;
                }
            }

            /* Dequeue any ready V4L2 frames */
            for (int p = 0; p < nfds; p++) {
                int si = pfd_to_src[p];
                struct source *s = &srcs[si];

                /* Detect disconnect */
                if (pfds[p].revents & (POLLERR | POLLHUP)) {
                    printf("V4L2: src%d %s error/hangup — will reconnect\n",
                           si, s->dev_path);
                    src_v4l2_cleanup(s);
                    s->active = 0;
                    continue;
                ...
                if (xioctl(s->fd, VIDIOC_DQBUF, &vb) < 0) {
                    if (errno == ENODEV) {
                        printf("V4L2: src%d %s removed — will reconnect\n",
                               si, s->dev_path);
                        src_v4l2_cleanup(s);
                        s->active = 0;
                    }
            ...
            for (int i = 0; i < MAX_SRCS; i++) {
                if (!srcs[i].active) continue;
                if (srcs[i].type == SRC_V4L2) continue;
                if (srcs[i].new_frame) {
                    pthread_mutex_lock(&srcs[i].lock);
                    last_bi[i] = srcs[i].front_idx;
                    srcs[i].new_frame = 0;
                    pthread_mutex_unlock(&srcs[i].lock);
                    frames++;
                    per_src_cnt[i]++;
                }
            }
            ...
            for (int i = 0; i < MAX_SRCS; i++) {
                ...
                if (!s->active) continue;
                int bi = last_bi[i];
                im_rect src_rect = { 0, 0, s->width, s->src_height };

                /* Compute zoom crop */
                im_rect zr = src_rect;
                if (s->cal.zoom > MIN_ZOOM) {
                    double zf = s->cal.zoom / 10.0;
                    int zw = (int)(s->width / zf), zh = (int)(s->src_height / zf);
                    if (zw < 64) zw = 64;
                    if (zh < 64) zh = 64;
                    int zx = (s->width - zw) / 2, zy = (s->src_height - zh) / 2;
                    zx &= ~1; zy &= ~1;
                    zr = (im_rect){ zx, zy, zw, zh };
                }

                /* Get the RGA source buffer for this source */
                rga_buffer_t src_rga;
                if (s->type == SRC_V4L2) {
                    src_rga = s->bufs[bi].rga;
                } else {
                    src_rga = s->nv12_rga[bi];
                }

                if (s->cal.k1 != 0.0 || s->cal.k2 != 0.0) {
                    /* RGA blit source -> intermediate, then GPU undistort to FBO */
                    improcess(src_rga, s->inter_rga, pat_empty,
                              src_rect, src_rect, prect_empty, IM_SYNC);
                    float crop[4] = {
                        zr.x / (float)s->width,
                        zr.y / (float)s->src_height,
                        zr.width / (float)s->width,
                        zr.height / (float)s->src_height,
                    };
                    ...
                    glUniform2f(gpu.loc_center, (float)s->cal.cx, (float)s->cal.cy);
                    glUniform2f(gpu.loc_res, (float)s->width, (float)s->src_height);
                    float max_r = sqrtf((float)(s->width * s->width +
                                                s->src_height * s->src_height)) * 0.5f;
                    glUniform1f(gpu.loc_max_r, max_r);
                    glUniform1f(gpu.loc_k1, (float)s->cal.k1);
                    glUniform1f(gpu.loc_k2, (float)s->cal.k2);
                    float fs = compute_fit_scale((float)s->cal.k1, (float)s->cal.k2,
                                                 (float)s->cal.cx, (float)s->cal.cy,
                                                 (float)s->width, (float)s->src_height, max_r);
                    glUniform1f(gpu.loc_fit_scale, fs);
                    glUniform4fv(gpu.loc_crop, 1, crop);
                    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
                    glFinish();
                } else {
                    /* Direct RGA blit to display quadrant */
                    improcess(src_rga, disp.rga[bb], pat_empty,
                              zr, s->quad, prect_empty, IM_SYNC);
                }
            }
            ...
            draw_quad_labels((uint32_t *)disp.map[bb], dst_stride4,
                             disp_w, disp_h, srcs, MAX_SRCS);
            draw_sys_stats((uint32_t *)disp.map[bb], dst_stride4,
                           disp_w, disp_h, gpu_pct, cpu_pct, fps);

            /* Screenshot (before flip so we capture the completed frame) */
            if (g_screenshot_req) {
                g_screenshot_req = 0;
                save_screenshot(disp.map[bb], disp.pitch, disp_w, disp_h);
            }
        ...
        /* ── MODE_SINGLE: fullscreen calibration for selected source ── */
            ...
            if (s->type == SRC_V4L2) {
                ...
                if (pfd.revents & (POLLERR | POLLHUP)) {
                    printf("V4L2: src%d %s error/hangup (single) — will reconnect\n",
                           sel, s->dev_path);
                    src_v4l2_cleanup(s);
                    s->active = 0;
                    goto fps_check;
                ...
                if (xioctl(s->fd, VIDIOC_DQBUF, &vb) < 0) {
                    if (errno == ENODEV) {
                        printf("V4L2: src%d %s removed (single) — will reconnect\n",
                               sel, s->dev_path);
                        src_v4l2_cleanup(s);
                        s->active = 0;
                    }
                ...
                bi = vb.index;
                got_frame = 1;
                /* Will requeue after render */
                last_bi[sel] = bi;
            } else {
                /* Wait for new decode frame */
                for (int w = 0; w < 20 && !quit && !s->new_frame; w++)
                    usleep(5000);
                if (s->new_frame) {
                    pthread_mutex_lock(&s->lock);
                    bi = s->front_idx;
            ...
            if (do_autodetect) {
                uint8_t *Y = extract_y_for_autodetect(s, bi);
                if (Y) {
                    auto_detect_edges(Y, s->width, s->src_height);
                    free(Y);
                    if (calib_npts >= 6) {
                        auto_fit_k1k2(s->width, s->src_height, cx, cy);
                        pthread_mutex_lock(&param_lock);
                        k1 = g_k1; k2 = g_k2;
                        pthread_mutex_unlock(&param_lock);
            ...
            if (zoom <= MIN_ZOOM) {
                ...
                int zcx = g_cursor_visible
                         ? g_cursor_x * s->width / disp_w
                         : s->width / 2;
                int zcy = g_cursor_visible
                         ? g_cursor_y * s->src_height / disp_h
                         : s->src_height / 2;
                int zx = zcx - zw / 2, zy = zcy - zh / 2;
                if (zx < 0) zx = 0;
                if (zy < 0) zy = 0;
                if (zx + zw > s->width) zx = s->width - zw;
                if (zy + zh > s->src_height) zy = s->src_height - zh;
                zx &= ~1; zy &= ~1;
            ...
            if (bypass || (k1 == 0.0 && k2 == 0.0)) {
                ...
                improcess(src_rga, s->inter_rga, pat_empty,
                          src_rect, src_rect, prect_empty, IM_SYNC);
                float crop[4] = {
                    zoom_rect.x / (float)s->width,
                    zoom_rect.y / (float)s->height,
                    zoom_rect.width / (float)s->width,
                    zoom_rect.height / (float)s->height,
                };
                glBindFramebuffer(GL_FRAMEBUFFER, gpu.fbo[bb]);
            ...
            draw_osd_single(disp.map[bb], disp.pitch, disp_w, disp_h,
                            fps, sel, s->label, k1, k2, cx, cy, grid_on, bypass);
            ...
            /* Requeue V4L2 buffer if applicable */
            if (s->type == SRC_V4L2) {
                struct v4l2_buffer vb = {
                    .type   = s->buf_type,
                    .memory = V4L2_MEMORY_MMAP,
            ...
            /* Drain other V4L2 sources to prevent queue stall */
            for (int i = 0; i < MAX_SRCS; i++) {
                if (i == sel || !srcs[i].active || srcs[i].type != SRC_V4L2) continue;
                struct v4l2_buffer dvb = {
                    .type   = srcs[i].buf_type,
                    .memory = V4L2_MEMORY_MMAP,
                };
                struct v4l2_plane dp[1] = {};
                if (srcs[i].is_mplane) { dvb.m.planes = dp; dvb.length = 1; }
                if (xioctl(srcs[i].fd, VIDIOC_DQBUF, &dvb) == 0)
                    xioctl(srcs[i].fd, VIDIOC_QBUF, &dvb);
            }
        }
...
fps_check:
        /* FPS + system stats */
        {
            ...
            if (elapsed >= 3.0) {
                ...
                if (g_mode == MODE_QUAD) {
                    ...
                    for (int i = 0; i < MAX_SRCS; i++) {
                        if (i) printf(" ");
                        printf("src%d:%.1f", i,
                               srcs[i].active ? per_src_cnt[i] / elapsed : 0.0);
                        per_src_cnt[i] = 0;
                    }
                    printf("]\n");
                }
            ...
            if (stats_elapsed >= 1.0) {
                ...
                /* Try to reconnect any disconnected V4L2 sources (once/sec) */
                for (int i = 0; i < MAX_SRCS; i++) {
                    if (srcs[i].active) continue;
                    if (inputs[i].type != SRC_V4L2) continue;
                    if (src_v4l2_reconnect(&srcs[i], i) == 0)
                        printf("src%d: reconnected %s\n", i, srcs[i].dev_path);
                }
            }
        }
    }
    ...
    /* Signal quit and join threads */
    quit = 1;
    for (int i = 0; i < MAX_SRCS; i++) {
        if (srcs[i].thread_started)
            pthread_join(srcs[i].dec_thread, NULL);
    }
    ...
    for (int i = 0; i < MAX_SRCS; i++) {
        if (srcs[i].active)
            gpu_cleanup_src_texture(&gpu, &srcs[i], disp.fd);
        if (srcs[i].type == SRC_V4L2 || srcs[i].type == SRC_V4L2_DECODE)
            src_v4l2_cleanup(&srcs[i]);
        if (srcs[i].type == SRC_V4L2_DECODE || srcs[i].type == SRC_RTSP)
            free_nv12_dumb(&srcs[i], disp.fd);
    }
